...
from modules.led.array.libsacn.messages.data_packet import DataPacket
from modules.led.array.libsacn.sending.output import Output
from modules.led.array.libsacn.sending.output_thread import OutputThread, DEFAULT_PORT


class sACNsender:
    def __init__(self, bind_address: str = "0.0.0.0", bind_port: int = DEFAULT_PORT,
                 source_name: str = "default source name", cid: tuple = (),
                 fps: int = 30, universe_discovery: bool = True,
        ...
        :param universe_discovery: if true, universe discovery messages are send out via broadcast every 10s.
        ...
        self._universe_discovery: bool = universe_discovery
        self._sync_universe: int = sync_universe

    @property
    def universe_discovery(self) -> bool:
        return self._universe_discovery
    @universe_discovery.setter
    def universe_discovery(self, universe_discovery: bool) -> None:
        self._universe_discovery = universe_discovery
        try:  # try to set the value for the output thread
            self._output_thread.universe_discovery = universe_discovery
        except:
            pass
    ...
    def start(self, bind_address=None, bind_port: int = None, fps: int = None) -> None:
        ...
        self._output_thread = OutputThread(cid=self.__CID, source_name=self.source_name,
                                           ...
                                           bind_port=bind_port, fps=fps, universe_discovery=self._universe_discovery)
